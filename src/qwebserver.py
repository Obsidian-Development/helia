...
from quart import Quart, redirect, render_template_string, request, url_for, session
...
app = Quart(__name__)
app.secret_key = b'\x92\xc2\x11\x9a\x87\xa85\t:iLjX\xd4\xe3\xbf\x9b\xf8s\x0b\xa7,\xda\xc4'
app.config['DISCORD_CLIENT_ID'] = "671612079106424862"
app.config['DISCORD_CLIENT_SECRET'] = 'Y_SI9mdgirATWZcb44KwLoF0WdVEhvN8'
...
async def callback():
    ...
    print(code)
    oauth = OAuth2Session(app.config['DISCORD_CLIENT_ID'], redirect_uri=app.config['DISCORD_REDIRECT_URI'])
    token = oauth.fetch_token(token_url, client_secret=app.config['DISCORD_CLIENT_SECRET'], code=code)
    print(token)
    data = {
