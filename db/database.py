...
class Database:
    ...
    def get_flagged_games(self, channel_id):
        ...
        flagged_games = [game[0] for game in games]
        self.conn.commit()
        self.conn.close()
    ...
    def get_game_channel(self, title):
        ...
        channels = [game[0] for game in games]
        self.conn.commit()
        self.conn.close()
        return channels
    ...
    def insert_coins(self, userid, coins, mention=None):
        ...
        if mention is None and self.bot != None: mention = self.bot.Client.get_user_info(userid)
    ...
    def get_coins(self, userid):
        ...
        output = sum(i[0] for i in result)
        self.conn.close()
    ...
    def get_top_coin_holders(self):
        ...
        toplist = [
            {"userid": user[0], "coins": user[1], "mention": user[2]}
            for user in result
        ]
